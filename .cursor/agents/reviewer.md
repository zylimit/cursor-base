...
3. When the delegation names a **lens**, report only that failure mode (correctness,
   ...
   privacy) and submit it with
   `node scripts/harness.mjs review lens <lens> --agent <your delegation id>`, findings as JSON
   on stdin. Every finding carries `severity` and a `file:line` location or a reproduction; an
   impression nobody can locate is not a finding. If you cannot conclude, report `unable` and
...
`Changed` must be `none`; `Evidence` includes the base commit, diff hash, and the lens command's response.
