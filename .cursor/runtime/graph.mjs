...
import { EXIT, boolOption, changedPaths, gitAvailable, gitBase, isWithin, matchesPath, normalizeLf, posix, printJson, readJson, targetFrom, walkFiles, writeJson, } from "./core.mjs";
...
export function requestedPaths(root, positional, options) {
    const named = [
        ...positional,
...
export function repoMap(options) {
...
export function affected(positional, options) {
    const root = targetFrom(options);
...
export function extractImports(file, contents) {
    const found = new Set();
...
export function resolveRelativeImport(root, fromFile, specifier) {
...
export function moduleForSpecifier(definition, specifier) {
    let best = null;
    let bestLength = -1;
    for (const module of definition.modules) {
...
export function layerViolation(definition, from, to) {
...
export function archCheck(options) {
    const root = targetFrom(options);
    const definition = catalog(root);
...
export function detectCommands(root) {
    ...
    if (exists("package.json")) {
        ...
        for (const [id, names, cls] of wanted) {
            ...
            if (hit)
                found.push({ id, command: `${runner} ${hit}`, class: cls, source: `package.json scripts.${hit}` });
        }
    }
...
function proposeModules(paths, depth) {
    ...
    for (const path of paths) {
        ...
        if (parts.length < 2 || NON_SOURCE.has(parts[0]))
            continue;
        let prefix = null;
    ...
    for (const path of paths) {
        if (covered.has(path))
            continue;
        const parts = path.split("/");
        if (parts.length < 2 || NON_SOURCE.has(parts[0]) || parts[0].startsWith("."))
            continue;
        ...
        if (!rest.has(top))
            rest.set(top, { patterns: new Set(), files: [] });
...
export function proposeAttributes(root, modules) {
    const proposals = {};
    for (const module of modules) {
        const hits = new Map();
        for (const file of module.files.slice(0, 300)) {
            if (!CODE_EXTENSIONS.has(extname(file).toLowerCase()) || NON_PRODUCTION.test(file))
                continue;
            let text;
            try {
                const raw = readFileSync(resolve(root, file));
                if (raw.length > 200_000 || raw.includes(0))
                    continue;
...
export function discoverCatalog(root, depth = 2) {
    const tracked = trackedPaths(root);
    if (!tracked.isGit)
        return { ok: false, degraded: true, reason: "not a git repository; the file set cannot be established" };
    if (tracked.paths.length === 0)
        return { ok: false, degraded: true, reason: "no tracked files; commit the project before discovering its structure" };
    ...
    const probe = { version: 1, modules: modules.map((module) => ({ id: module.id, paths: module.paths })) };
        ...
        for (const file of module.files) {
            ...
            for (const specifier of extractImports(file, contents)) {
                ...
                if (target && target.id !== module.id)
                    edges.get(module.id).add(target.id);
            }
        }
    }
    ...
    const draftBase = { version: 1, modules: [], globalPaths, ignored };
    ...
    for (const path of unmapped.filter((candidate) => !candidate.includes("/")))
...
export function writeDiscoveredCatalog(root, result) {
    ...
    for (const [live, template, schema, value] of targets) {
        ...
        writeJson(resolve(root, target), { $schema: schema, ...value });
        written.push(target);
    }
    return written;
}
export function catalogDiscover(options) {
    const root = targetFrom(options);
    ...
    const written = boolOption(options, "write")
