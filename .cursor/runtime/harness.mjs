...
import { appendFileSync, closeSync, cpSync, existsSync, openSync, readSync, lstatSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, realpathSync, renameSync, rmSync, statSync, writeFileSync, } from "node:fs";
...
function moduleEffectivePaths(module) {
...
function classifyPath(definition, path) {
...
const ATTRIBUTE_TIERS = ["critical", "high", "medium", "low", "minimal", "none"];
...
function isHarnessSourceRoot(root) {
...
function withStateLock(root, name, action) {
...
function diffArgumentSets(base) {
...
function withDiffOutput(args, output) {
...
function normalizeLfChunk(chunk, carry) {
...
function canonicalDiffDigest(cwd, base) {
    if (!gitAvailable(cwd)) {
...
function canonicalDiffText(cwd, base, limit) {
    if (!gitAvailable(cwd))
        return "";
...
function walkFiles(root, current = root, excludeTests = false) {
...
function matrix(root) {
    ...
    return readJson(existsSync(local) ? local : resolve(HARNESS_ROOT, "harness/default-verification-matrix.json"));
}
...
function affectedModules(root, paths, discovered = true) {
    const definition = catalog(root);
...
function requestedPaths(root, positional, options) {
    ...
    if (named.length > 0)
        return { paths: named.map(posix), explicit: true };
...
function buildVerifyPlan(root, positional, options) {
    ...
    if (impact.unmatched.length > 0) {
        ...
        if (seen.has(conservativeId)) {
            ...
            if (existing)
                existing.conservative = true;
        }
        else {
            selected.push({ id: conservativeId, ...checks[conservativeId], conservative: true });
        }
    }
    const modules = impact.affected.map((module) => module.id);
...
function executeCheck(root, check, plan) {
    ...
    const spawnCode = result.error ? result.error.code : undefined;
...
function signReceipt(receipt) {
    receipt.content_sha256 = contentHash(receipt, "content_sha256");
    return receipt;
}
function readQualityLedger(root) {
...
function assessQuality(root, plan) {
    ...
    const checks = plan.checks.map((check) => {
        ...
        const matching = ledger.filter((receipt) => receipt.check_id === check.id &&
    ...
    for (const moduleId of plan.modules) {
        ...
        for (const [attribute, requirement] of Object.entries(module?.attributes || {})) {
            ...
            const claiming = (module?.verification || []).filter((checkId) => (plan.checks.find((check) => check.id === checkId)?.attributes || []).includes(attribute));
            const evidence = claiming.map((checkId) => ({
                check: checkId,
                status: statusOf.get(checkId) ?? "MISSING",
            }));
            ...
            const contradicting = evidence.filter((entry) => entry.status === "FAIL" || entry.status === "BLOCKED");
            attributes.push({
                module: moduleId,
                attribute: attribute,
                tier,
...
function gate(positional, options) {
    const root = targetFrom(options);
    ...
    if (wanted.length === 0) {
        ...
        printJson({
            ...
            modules: plan.modules,
            status: "BLOCKED",
            reason: "The verification plan selected no checks. Either nothing changed, or the catalog excludes the changed paths.",
            results: [],
        });
        process.exitCode = 2;
        return;
...
function moduleForPath(definition, path) {
    let best = null;
    let bestScore = -1;
    for (const module of definition.modules) {
        for (const pattern of moduleEffectivePaths(module)) {
...
function moduleForSpecifier(definition, specifier) {
    let best = null;
    let bestLength = -1;
    for (const module of definition.modules) {
...
function layerViolation(definition, from, to) {
    const order = definition.layers || [];
    if (order.length === 0 || !from.layer || !to.layer)
        return null;
...
function archCheck(options) {
    const root = targetFrom(options);
    const definition = catalog(root);
    const maxFiles = Number(options["max-files"]) > 0 ? Number(options["max-files"]) : 20_000;
    const declared = new Map(definition.modules.map((module) => [module.id, new Set(module.dependsOn || [])]));
    const actual = new Map(definition.modules.map((module) => [module.id, new Set()]));
        ...
        for (const specifier of extractImports(rel, contents)) {
            ...
            if (forbiddenRule) {
                ...
                if (existing.evidence.length < 5)
                    existing.evidence.push(site);
                forbidden.set(key, existing);
                continue;
            }
            if (declared.get(owner.id)?.has(target.id))
                continue;
    ...
    for (const [id, targets] of declared) {
        for (const target of targets) {
            if (!actual.get(id)?.has(target))
                unusedDeclarations.push({ from: id, to: target });
        }
    }
    ...
    printJson({
        ...
        forbidden_dependencies: [...forbidden.values()].sort((left, right) => `${left.from}->${left.to}`.localeCompare(`${right.from}->${right.to}`)),
        undeclared_dependencies: [...violations.values()].sort((left, right) => `${left.from}->${left.to}`.localeCompare(`${right.from}->${right.to}`)),
        ...
        unused_declarations: unusedDeclarations.sort((left, right) => `${left.from}->${left.to}`.localeCompare(`${right.from}->${right.to}`)),
        cycles,
    });
    if (!ok)
        process.exitCode = 1;
...
function readForContext(root, rel, limit) {
...
function contextPack(positional, options) {
    const root = targetFrom(options);
    const definition = catalog(root);
    const budget = {
    ...
    for (const module of impact.affected) {
        ...
        if (entry)
            candidates.push({ ...entry, priority: 1 });
    }
    ...
    if (included.length === 0 && ordered.length > 0) {
        throw new Error("Context budget is too small to include anything; raise the budget or narrow the change set.");
    }
    const packHash = sha256(canonicalJson({
        budget,
...
function preflightTaskWrite(root, rel) {
    const task = activeTask(root);
    if (!task || !ownsPath(task, rel))
        return null;
    const current = fileHash(resolve(root, rel));
    if (!Object.hasOwn(task.known_hashes, rel)) {
        if (current === null)
            return null;
        return decision("deny", `Owned path ${rel} exists but was not captured by the task baseline. Restart the task or escalate before overwriting it.`, rel);
    }
    if (current !== task.known_hashes[rel]) {
        return decision("deny", `${rel} changed outside this task since it was last written. Reconcile the conflict before writing again.`, rel);
    }
    return null;
}
function recordTaskWrite(root, rel) {
...
function taskCommand(positional, options) {
    const root = targetFrom(options);
    ...
    if (subcommand === "start") {
        const owned = String(options.owned || "")
            ...
            .filter(Boolean);
        if (!options.goal)
            throw new Error("task start requires --goal.");
        if (owned.length === 0)
            throw new Error("task start requires --owned with at least one path glob.");
        ...
        return withStateLock(root, "tasks", () => {
            ...
            for (const absolute of walkFiles(root)) {
                const rel = posix(relative(root, absolute));
                if (matchesPath(rel, owned))
                    knownHashes[rel] = fileHash(absolute);
            }
            const task = {
                version: 1,
    ...
    if (subcommand === "cancel" || subcommand === "complete") {
        ...
        const completionAssessment = subcommand === "complete" ? assessQuality(root, buildVerifyPlan(root, [], options)) : null;
        return withStateLock(root, "tasks", () => {
            const state = readTasks(root);
            const task = state.tasks.find((entry) => entry.status === "active");
            if (!task)
...
const DEFAULT_FITNESS_RULES = [
    ...
    },
    {
        ...
        rationale: "A retry loop with no bound or backoff turns a transient fault into a sustained outage. This is a heuristic; suppress it on a loop that does bound its attempts.",
...
function fitness(options) {
    ...
    const subjects = scanAll
        ...
        : requestedPaths(root, [], options).paths.filter((path) => SOURCE_EXTENSIONS.has(path.slice(path.lastIndexOf("."))));
    const findings = [];
    let scanned = 0;
    for (const path of subjects) {
        const absolute = resolve(root, path);
        if (contextDenied(path) || !existsSync(absolute))
            continue;
        let stats;
        try {
            stats = lstatSync(absolute);
        ...
        for (const rule of rules) {
            if (rule.appliesTo?.length && !matchesPath(path, rule.appliesTo))
                continue;
            if (rule.modules?.length && (!owner || !rule.modules.includes(owner.id)))
                continue;
            // A rule scoped to attributes the module opted out of should not fire at all.
            if (owner && ruleOptedOut(owner, rule))
                continue;
            if (rule.minimumTier && !meetsMinimumTier(owner, rule))
                continue;
            if (rule.require) {
            ...
            while (match) {
                ...
                const suppressed = (lines[line - 1] || "").includes(FITNESS_IGNORE) ||
...
function adapters(positional, options) {
    const root = targetFrom(options);
    ...
    if (subcommand === "list") {
        ...
        const filtered = wanted
            ? catalogue.filter((adapter) => adapter.attributes.includes(wanted))
            : catalogue;
    ...
    if (subcommand !== "add")
        throw new Error("adapters supports list or add.");
    const id = positional[1] || String(options.id || "");
    const adapter = catalogue.find((entry) => entry.id === id);
    if (!adapter)
        throw new Error(`Unknown adapter: ${id || "<missing>"}. Run \`adapters list\`.`);
    const matrixPath = resolve(root, "harness/verification-matrix.json");
    ...
    if (!boolOption(options, "dry-run"))
        writeJson(matrixPath, matrixValue);
    printJson({
...
function adrCheck(options) {
    ...
    printJson({
        ...
        failing: failing.map((record) => ({
            ...
            reason: record.enforced_by.length === 0
...
function quality(positional, options) {
    ...
    if (subcommand === "attributes") {
        ...
        const byTier = {};
        for (const entry of assessment.attributes)
        ...
        if (gaps.some((entry) => entry.enforcement === "block"))
            process.exitCode = 2;
        return;
    }
...
function stripExecutableName(token) {
...
function parseShellCommand(value) {
    const segments = [];
    let tokens = [];
    let current = "";
...
function looksLikePathspec(root, token) {
...
function classifyGit(rest, raw, root) {
...
function classifySegment(segment, raw, root) {
...
function sensitiveTokens(segment) {
    const found = [];
...
function classifySecretExposure(parse, raw) {
...
function shellDecision(command, root) {
...
function rotateLedgerIfLarge(path) {
    try {
...
function appendLedger(root, event, payload, outcome, reason) {
...
async function handleHookEvent(event, payload, root) {
    let output = {};
    if (event === "beforeShellExecution") {
        ...
        output = {
            additional_context: `Harness state was saved to ${posix(relative(root, notePath))} before compaction: base commit ${current.base_commit}, ` +
        ...
        if (payload.status === "completed" &&
            ...
            if (!assessment.complete) {
                ...
                output = {
                    followup_message: `The current diff has ${parts.join("; and ")}. ` +
...
function compareCompiledRuntime(root, errors) {
...
function validateReceipt(value) {
    ...
    for (const field of required) {
        if (!Object.hasOwn(value || {}, field))
            errors.push(`Missing receipt field: ${field}.`);
    }
    if (value?.version !== 1)
    ...
    if (typeof value?.not_reviewed !== "string")
        errors.push("Receipt not_reviewed must be a string.");
    if (!validTimestamp(value?.created_at))
        errors.push("Receipt created_at must be an ISO timestamp.");
...
function receipt(positional, options) {
    ...
    if (subcommand === "check") {
        ...
        if (!positional[1] && !options.file)
            throw new Error("receipt check requires a receipt path.");
        const value = readJson(path);
...
function waiver(positional, options) {
    ...
    if (subcommand === "check") {
        const path = resolve(root, String(positional[1] || options.file || ""));
        if (!positional[1] && !options.file)
            throw new Error("waiver check requires a waiver path.");
        const value = readJson(path);
...
function validateWaiver(value) {
    ...
    if (value?.expiry) {
        ...
        if (!validTimestamp(value.expiry))
            errors.push("Waiver expiry must be an ISO timestamp.");
...
export async function main(argv = process.argv.slice(2)) {
    ...
    switch (command) {
        ...
        case "arch-check":
            ...
            break;
        case "catalog":
            if (positional[0] && positional[0] !== "lint")
                throw new Error("catalog supports the lint subcommand.");
            catalogLint(options);
            break;
        case "context-pack":
