...
import { EXIT, STATE_REL, binding, boolOption, changedPaths, git, gitAvailable, gitBase, posix, printJson, readJson, stdinJson, targetFrom, withStateLock, writeJson, } from "./core.mjs";
...
function sessionBinding(root, session) {
 ...
 */
export function authorsOf(root, paths, base) {
...
export async function authorshipCommand(positional, options) {
    ...
    if (subcommand === "show") {
        ...
        const files = session ? changedFiles(root, session) : gitAvailable(root) ? changedFiles(root, { base_commit: base }) : [];
        printJson({
