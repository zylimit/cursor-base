...
import { EXIT, HARNESS_ROOT, binding, changedPaths, git, gitAvailable, gitBase, isHarnessSourceRoot, normalizeLf, printJson, run, targetFrom, whichCommand, } from "./core.mjs";
...
export function releaseReadiness(root, options) {
    ...
    try {
        ...
        if (plan.checks.length === 0)
            add("plan", "BLOCKED", "the verification plan selected no checks; a release with nothing to verify is not verified");
    }
    catch (error) {
        add("gate", "BLOCKED", `the verification plan could not be built: ${error.message}`);
    }
    ...
    try {
        ...
        add("memory", "BLOCKED", `memory could not be read: ${error.message}`, false);
    }
    ...
    if (gitAvailable(root)) {
        ...
        if (!hasRemote)
            add("ci", "BLOCKED", "no git remote is configured, so the CI status of HEAD cannot be observed", false);
        else if (!whichCommand("gh"))
            add("ci", "BLOCKED", "gh is not installed, so the CI status of HEAD could not be read", false);
        else {
            ...
            if (!result.ok)
                add("ci", "BLOCKED", `gh could not list runs for ${sha.slice(0, 12)}: ${normalizeLf(result.stderr).trim().slice(0, 200)}`, false);
            else {
                let runs = [];
                try {
                ...
                if (runs.length === 0)
                    add("ci", "BLOCKED", `no CI run exists for ${sha.slice(0, 12)}`, false);
                else if (runs.some((entry) => entry.status !== "completed"))
                    add("ci", "BLOCKED", "a CI run for HEAD is still in progress", false);
                else if (runs.every((entry) => entry.conclusion === "success"))
                    add("ci", "PASS", `${runs.length} CI run(s) succeeded for HEAD`, false);
                else
                    add("ci", "FAIL", `CI did not succeed for HEAD: ${runs.filter((entry) => entry.conclusion !== "success").map((entry) => `${entry.name}=${entry.conclusion}`).join(", ")}`, false);
            }
        }
    }
...
export function releaseCommand(positional, options) {
    ...
    if (!result.ready)
        process.exitCode = failing.length > 0 ? EXIT.GATE : EXIT.DEGRADED;
}
