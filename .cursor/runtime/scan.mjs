...
export const DEFAULT_FITNESS_RULES = [
    {
...
export function fitnessRules(root) {
...
export function meetsMinimumTier(module, rule) {
    if (!module)
        return false;
...
// for it at all.
export function ruleOptedOut(module, rule) {
...
export function adapterCatalog(root) {
...
export function adapters(positional, options) {
    const root = targetFrom(options);
...
export function adrCheck(options) {
    const root = targetFrom(options);
...
export function scanInstructions(root, files) {
    ...
    for (const file of files) {
        ...
        try {
            const raw = readFileSync(absolute);
            if (raw.length > 1024 * 1024 || raw.includes(0))
                continue;
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            if (line.includes(INSTRUCTION_IGNORE) || (index > 0 && lines[index - 1].includes(INSTRUCTION_IGNORE)))
                continue;
...
export function instructionsCommand(options) {
    const root = targetFrom(options);
...
export function skillsLint(root) {
    const findings = [];
    const skills = [];
    for (const skillRoot of SKILL_ROOTS) {
        ...
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (!name)
                findings.push({ file, severity: "error", code: "NO_NAME", message: "frontmatter requires name" });
            else {
                if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(name))
                    findings.push({ file, severity: "error", code: "NAME_NOT_KEBAB", message: `skill name ${JSON.stringify(name)} must be lowercase letters, numbers, and hyphens` });
                if (name !== entry.name)
                    findings.push({ file, severity: "error", code: "NAME_MISMATCH", message: `frontmatter name ${JSON.stringify(name)} does not match the folder ${JSON.stringify(entry.name)}` });
            }
            const description = meta.description ?? "";
            if (!description)
                findings.push({ file, severity: "error", code: "NO_DESCRIPTION", message: "frontmatter requires description; it is the only routing signal the agent sees before loading the skill" });
            else if (description.length > 500)
                findings.push({ file, severity: "error", code: "DESCRIPTION_TOO_LONG", message: `description is ${description.length} characters; keep it under 500` });
            else if (description.length > 220)
                findings.push({ file, severity: "warning", code: "DESCRIPTION_LONG", message: `description is ${description.length} characters; every session pays for it on every request` });
            ...
            if (bytes > 24_000)
                findings.push({ file, severity: "warning", code: "SKILL_LARGE", message: `skill body is ${bytes} bytes and is paid in full on load; move detail into a REFERENCE.md` });
...
export function skillsLintCommand(options) {
    const root = targetFrom(options);
...
export function agentsLint(root) {
    const findings = [];
    const contracts = [];
    ...
    for (const module of catalog(root).modules) {
        ...
        if (missing.length)
            findings.push({ module: module.id, file, severity: "warning", code: "MODULE_AGENTS_INCOMPLETE", message: `${file} is missing section(s): ${missing.join(", ")}` });
        if (bytes > ROOT_AGENTS_BUDGET_BYTES)
            findings.push({ module: module.id, file, severity: "warning", code: "MODULE_AGENTS_LARGE", message: `${file} is ${bytes} bytes` });
    }
    return { contracts, findings };
}
export function agentsLintCommand(options) {
    const root = targetFrom(options);
...
export function rulesAudit(root, knownCommands, files = null) {
    const known = new Set(knownCommands);
    for (const id of Object.keys(matrix(root).checks || {}))
        known.add(id);
    for (const rule of fitnessRules(root))
        known.add(rule.id);
    for (const event of EVENTS)
        known.add(event);
    const targets = files ?? ["AGENTS.md", ...listRuleFiles(root)];
    const rows = [];
    const tokenOf = (raw) => raw
        .trim()
    ...
    for (const file of targets) {
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            for (const match of rule.matchAll(/`([^`]{2,90})`/g)) {
                ...
                if (!token || token.includes("<") || raw.includes("...") || token.startsWith("--"))
                    continue;
                if (known.has(token)) {
                    enforcedBy.push(token);
                    continue;
                }
                if (existsSync(resolve(root, token)) || existsSync(resolve(root, ".cursor", token)))
                    continue;
                // Something spelled like a harness command or a script path that does not exist.
                if (/^node\s+(scripts\/harness\.mjs|\.cursor\/runtime\/harness\.mjs)\s+/.test(raw.trim()))
                    phantoms.push(raw);
                else if (/^(\.cursor|scripts|harness)\//.test(token) || /\.(sh|ps1|mjs|js|ya?ml)$/.test(token))
                    phantoms.push(raw);
            }
...
export function rulesAuditCommand(options, knownCommands) {
    const root = targetFrom(options);
