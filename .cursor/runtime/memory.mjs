...
export function memoryConfig(root) {
    let declared = {};
    try {
 ...
 */
export function ledgerEntries(section) {
    if (!section)
        return [];
    const entries = [];
...
export function sectionNamed(sections, name) {
    const lower = name.toLowerCase();
    return sections.find((section) => section.title.toLowerCase().startsWith(lower)) ?? null;
}
const clip = (line, max = 220) => line.length <= max ? line : `${line.slice(0, max - 3).trimEnd()}...`;
const firstLines = (entries, limit) => entries.slice(0, limit).map((entry) => clip(entry[0]));
export function ledgerHealth(root) {
...
export function recap(root, budget = null) {
    const config = memoryConfig(root);
    ...
    if (!present)
        push("Memory", [`- ${config.ledger} is absent; nothing to recall. Create it from the project-memory skill contract.`]);
...
export function invariants(root, budget = 1200) {
    const live = liveState(root);
    const state = [
 ...
 */
export function syncCheck(root, changed) {
    const config = memoryConfig(root);
...
// ============================== archive ==============================
export function archiveLedger(root, apply) {
    const config = memoryConfig(root);
    const path = resolve(root, config.ledger);
    if (!existsSync(path))
    ...
    if (total === 0)
        return { ok: true, applied: false, moved: 0, plan, health: ledgerHealth(root) };
    if (!apply)
        return { ok: true, applied: false, moved: total, plan, health: ledgerHealth(root) };
...
export function invariantsCommand(options) {
    const root = targetFrom(options);
...
export function syncCheckCommand(options) {
    const root = targetFrom(options);
...
export function feedbackLessons(root) {
...
export function feedbackCommand(positional, options) {
    const root = targetFrom(options);
