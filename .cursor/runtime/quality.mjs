...
export function buildVerifyPlan(root, positional, options) {
    ...
    const planModules = breadth === "none"
        ? []
    ...
    const modules = planModules.map((module) => module.id);
    ...
    return {
        ...
        plan_sha256: sha256(canonicalJson({
            modules,
            risk: taskRisk,
...
export function verifyPlan(positional, options) {
...
export function verifyLedgerChain(root) {
 ...
 */
export function deferrable(check, plan) {
    if (!check.allowFastSkip)
        return false;
    if (plan.assurance.controls.deferral !== "loan")
        return false;
    if (isProtectedCheck(check))
        return false;
...
export function readQualityLedger(root) {
    const path = resolve(root, QUALITY_LEDGER_REL);
    if (!existsSync(path))
        return [];
    const value = readJson(path);
...
export function appendQualityLedger(root, receipts) {
...
export const WAIVABLE_STATUSES = new Set(["MISSING", "BLOCKED", "SKIPPED"]);
export function waiverFor(waivers, checkId, diffSha256) {
...
export function checkClaimsCritical(definition, moduleIds, check) {
...
export function consecutiveFailures(ledger, checkId) {
    let streak = 0;
...
export function assessQuality(root, plan) {
    ...
    if (gapsBlock)
        for (const gap of blockingGaps)
            blockers.push(`attribute ${gap.module}/${gap.attribute} (${gap.tier}) is uncovered`);
    else
        for (const gap of blockingGaps)
    ...
    if (loaned.length > 0)
        blockers.push(`checks deferred on this diff: ${loaned.map((check) => check.id).join(", ")}`);
...
export function acceptingReceipt(root, bound, options = {}) {
    ...
    for (const name of candidates) {
        ...
        if (validateReceipt(value).length > 0)
            continue;
...
function reviewRequirement(root, plan) {
...
export function gate(positional, options) {
    ...
    if (wanted.length === 0) {
        ...
        printJson({
            ...
            modules: plan.modules,
            status: "BLOCKED",
            reason: plan.assurance.controls.verificationBreadth === "none"
    ...
    recordDebts(root, deferred.map((receipt) => ({
    ...
    const statusCounts = { PASS: 0, FAIL: 0, BLOCKED: 0, SKIPPED: 0 };
...
export function receipt(positional, options) {
    const root = targetFrom(options);
...
export function writeReviewReceipt(root, request) {
    const value = {
        version: 1,
...
export function waiver(positional, options) {
    const root = targetFrom(options);
...
export function validateWaiver(value) {
    const errors = [];
    if (value?.version !== 1)
