...
import { adapters, adrCheck, agentsLintCommand, fitness, instructionsCommand, rulesAuditCommand, skillsLintCommand, } from "./scan.mjs";
    ...
    switch (command) {
        ...
        case "catalog":
            if (positional[0] === "discover")
                catalogDiscover(options);
            else if (!positional[0] || positional[0] === "lint")
                catalogLint(options);
            else
                throw new Error("catalog supports the lint or discover subcommand.");
            break;
        case "context-pack":
