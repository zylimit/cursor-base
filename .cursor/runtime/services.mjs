...
export const SERVICE_RESTART_DEFAULTS = {
    backoffMs: 500,
    maxBackoffMs: 30_000,
    maxRestarts: 10,
    windowSec: 600,
...
export function servicesConfig(root) {
...
export function serviceDir(root, name) {
    return resolve(root, SERVICES_STATE_REL, name);
}
export function readServiceState(root, name) {
...
export function synthesizeServiceStatus(state) {
    if (!state)
...
export function appendServiceLog(root, name, text) {
...
export function supervisorLog(root, name, message) {
    appendServiceLog(root, name, `[supervisor ${new Date().toISOString()}] ${message}\n`);
}
export function probeHealth(url, timeoutMs) {
...
export async function serviceSupervise(root, name) {
...
export async function serviceStart(root, name) {
    const definition = servicesConfig(root)[name];
    if (!definition)
        throw new Error(`Service ${name} is not defined in ${SERVICES_CONFIG_REL}.`);
...
export async function serviceStop(root, name) {
...
export function serviceStatus(root, name) {
...
export function listServiceStateDirs(root) {
...
export function serviceLogs(root, name, options) {
...
export async function serviceCommand(positional, options) {
