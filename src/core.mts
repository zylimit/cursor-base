...
export type CliOptions = Record<string, OptionValue>;
...
export function findHarnessRoot(start: string): string {
...
export function isHarnessSourceRoot(root: string): boolean {
...
export function boolOption(options: CliOptions, key: string): boolean {
  const value = options[key];
  return value === true || value === "true" || value === "1";
}

export function targetFrom(options: CliOptions): string {
...
export function git(cwd: string, args: string[], allowFailure = false): ProcessResult {
  return run("git", args, cwd, allowFailure);
}

export const gitReadiness = new Map<string, boolean>();
...
export function gitBase(cwd: string, requested?: OptionValue): string {
 ...
 */
export function diffStats(cwd: string, base: string): { changed_lines: number; new_files: number } | null {
  ...
  for (const args of passes) {
    ...
    for (const record of result.stdout.split("\0")) {
      ...
      if (!match || !excludeState(posix(match[3]))) continue;
...
export function withDiffOutput(args: string[], output: string): string[] {
...
export function normalizeLfChunk(chunk: Buffer, carry: boolean): { text: string; carry: boolean } {
...
export function canonicalDiffDigest(cwd: string, base: string): string {
  if (!gitAvailable(cwd)) {
    return sha256(
      normalizeLf(
...
export function splitNulPaths(value: string): string[] {
...
export function canonicalDiffText(cwd: string, base: string, limit: number): string {
  if (!gitAvailable(cwd)) return "";
...
export const bindingCache = new Map<string, DiffBinding>();

export function binding(cwd: string, requestedBase?: OptionValue): DiffBinding {
...
export const GLOB_REGEX_CACHE = new Map<string, RegExp>();
...
export function walkFiles(root: string, current: string = root, excludeTests = false): string[] {
 ...
 */
export const LIVE_CONTRACTS: ReadonlyArray<readonly [live: string, template: string]> = [
...
export function snapshotFiles(root: string, installableOnly = false): FileEntry[] {
...
export function copyNormalized(source: string, destination: string): void {
...
export function pruneDirectory(directory: string, keep: number): void {
  try {
...
export async function stdinJson(): Promise<HookPayload> {
  let input = "";
...
export function contentHash(value: Record<string, unknown>, field: string): string {
  const copy = { ...value };
  delete copy[field];
