...
export interface ShellParse {
   ...
   * only those outside single quotes, where the same characters are literal. Classified like
   * top-level commands, so a machine command cannot hide behind `echo $(...)`.
   */
 ...
 */
export function classifySubstitutions(parse: ShellParse, raw: string, root?: string, depth = 0): HookOutput {
  let verdict: HookOutput = { permission: "allow" };
  if (parse.substitutions.length === 0) return verdict;
  ...
  for (const inner of parse.substitutions) {
    ...
    for (const segment of nested.segments) verdict = strictest(verdict, classifySegment(segment, raw, root));
