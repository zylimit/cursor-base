...
export interface ShellParse {
  ...
  dynamic: boolean;
  /**
   * True when something only a shell can resolve is present: an unquoted or double-quoted
   * variable or substitution, an unquoted glob, brace or tilde expansion, a redirection, or
   * grouping parentheses. Such a command must run through a shell; spawning it directly would
   * pass `$PORT` or `*.js` to the program literally.
   */
  expands: boolean;
  /**
   * The command text inside each `$(...)` and backtick substitution that a shell would run —
   * only those outside single quotes, where the same characters are literal. `classifyParsed`
   * walks each of these with the same rule set as the outer command.
   */
...
export type SpawnTarget =
  | { kind: "direct"; program: string; args: string[]; env?: Record<string, string> }
  | { kind: "shell" }
...
export function peelAssignments(tokens: string[]): { env: Record<string, string>; rest: string[] } {
...
export function directSpawnTarget(parse: ShellParse, cwd: string): SpawnTarget {
  ...
  if (rest.length === 0) return { kind: "shell" };
  const program = rest[0];
  if (SHELL_KEYWORDS.has(program.toLowerCase())) return { kind: "shell" };
  const fileAt = (candidate: string): string | null => {
    try {
...
export const COMMAND_WRAPPERS = new Map<string, number>([
  ["sudo", 0],
  ["doas", 0],
  ["nohup", 0],
  ["time", 0],
...
export function stripExecutableName(token: string): string {
...
export function parseShellCommand(value: string): ShellParse {
  ...
  const captureSubstitution = (index: number) => {
    if (value[index] === "`") {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (quote === '"') {
      if (char === '"') quote = null;
        ...
        if (char === "$" && value[index + 1] === "(") captureSubstitution(index);
        // Backticks pair across quote contexts; only the opener starts a substitution.
        if (char === "`") {
          if (!backtickOpen) captureSubstitution(index);
...
export function gitOperation(args: string[]): { subcommand: string; rest: string[] } {
  let index = 0;
...
export function severity(permission: Permission | undefined): number {
  if (permission === "deny") return 2;
  if (permission === "ask") return 1;
  return 0;
}

export function strictest(left: HookOutput, right: HookOutput): HookOutput {
...
export function looksLikePathspec(root: string | undefined, token: string): boolean {
  if (token === "." || token.endsWith("/")) return true;
  if (root && existsSync(resolve(root, token))) return true;
  return /\.[A-Za-z0-9]{1,6}$/.test(token);
}

export function classifyGit(rest: string[], raw: string, root?: string): HookOutput | null {
...
function isMachineCommand(name: string): boolean {
...
export function classifyPatterns(text: string, subject: string): HookOutput {
...
export function classifyParsed(
  ...
  let verdict: HookOutput = { permission: "allow" };
...
export function pathFragments(token: string): string[] {
...
export function sensitiveTokens(segment: ShellSegment): string[] {
  const found: string[] = [];
...
export function classifySecretExposure(parse: ShellParse, raw: string): HookOutput {
...
export function shellDecision(command: unknown, root?: string): HookOutput {
  const value = String(command || "").trim();
  if (!value) return { permission: "allow" };
  return classifyParsed(parseShellCommand(value), value, root);
}

export function mcpDecision(payload: HookPayload): HookOutput {
...
export function decision(permission: Permission, message: string, subject: string): HookOutput {
  return {
    permission,
...
export function toolPaths(input: Record<string, unknown>): string[] {
  const found: string[] = [];
