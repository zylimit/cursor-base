...
import type { AttributeTier, ModuleDefinition, QualityAttribute, RiskLevel } from "./catalog.mjs";
import {
  EXIT,
  STATE_REL,
  boolOption,
...
export const PROFILE_ORDER = ["explore", "rapid", "balanced", "strict"] as const;

export type BuiltinProfile = (typeof PROFILE_ORDER)[number];

/** Attributes that no profile, loan, or waiver may weaken. */
export const PROTECTED_ATTRIBUTES: readonly QualityAttribute[] = ["security", "safety", "privacy"];
...
} as const satisfies Record<string, { stage: 1 | 2 | 3; attribute: QualityAttribute | null }>);
...
export type ControlName = keyof typeof CONTROL_SCALES;

export interface AssuranceControls {
  verificationBreadth: (typeof CONTROL_SCALES.verificationBreadth)[number];
  deferral: (typeof CONTROL_SCALES.deferral)[number];
  reviewMode: (typeof CONTROL_SCALES.reviewMode)[number];
  attributeGaps: (typeof CONTROL_SCALES.attributeGaps)[number];
  memorySync: (typeof CONTROL_SCALES.memorySync)[number];
  budget: (typeof CONTROL_SCALES.budget)[number];
  contextDepth: (typeof CONTROL_SCALES.contextDepth)[number];
  completion: (typeof CONTROL_SCALES.completion)[number];
  /** Lenses a structured review must convene; a superset is stronger. */
  reviewLenses: ReviewLens[];
}

const CONTROL_NAMES = Object.keys(CONTROL_SCALES) as ControlName[];

export const BUILTIN_PROFILES: Readonly<Record<BuiltinProfile, AssuranceControls>> = Object.freeze({
  explore: {
  ...
  strict: {
    ...
    reviewLenses: Object.keys(REVIEW_LENSES) as ReviewLens[],
  },
});
...
export interface AssurancePolicy {
  version: 1;
  /** Profile used when the selection is `adaptive` and no floor raises it. */
  default: string;
  /** Longest fast loan a project allows; the hard ceiling is 24 hours. */
  maxLoanMinutes: number;
  /** Named profiles that extend a built-in and may only tighten it. */
  profiles: Record<string, { extends: string; controls: Partial<AssuranceControls> }>;
  floors: {
    risk: Record<RiskLevel, string>;
    /** Applied when the change hits unmapped, global, overlapping, or shared paths. */
    impact: string;
    /** Applied when an affected module holds a protected attribute at critical or high. */
    protectedAttributes: string;
    /** Applied when an affected module holds any other attribute at critical or high. */
    criticalHighAttributes: string;
    paths: PathFloor[];
  };
}
...
export const DEFAULT_POLICY: AssurancePolicy = Object.freeze({
  version: 1,
  default: "balanced",
  maxLoanMinutes: 480,
  profiles: {},
...
export interface CompiledPolicy {
  ...
  /** Every profile, built-in and named, with its effective controls. */
  profiles: Map<string, AssuranceControls>;
  /** Built-in rank each profile reduces to, for reporting the effective strength. */
  ranks: Map<string, number>;
  definition_sha256: string;
  source: string;
}

function rank(name: string, scale: readonly string[]): number {
  const index = scale.indexOf(name);
  if (index < 0) throw new Error(`Unknown value ${name}; expected one of ${scale.join(", ")}.`);
  return index;
}

/** Control names on which `candidate` is weaker than `baseline`. */
export function weakerControls(candidate: AssuranceControls, baseline: AssuranceControls): string[] {
  const weaker: string[] = [];
  for (const name of CONTROL_NAMES) {
    const scale = CONTROL_SCALES[name] as readonly string[];
    if (rank(candidate[name], scale) < rank(baseline[name], scale)) weaker.push(name);
  }
...
function strongerOf(left: AssuranceControls, right: AssuranceControls): AssuranceControls {
  ...
  return merged as unknown as AssuranceControls;
}

function validateControls(value: unknown, label: string): Partial<AssuranceControls> {
  ...
  for (const [name, raw] of Object.entries(controls)) {
    ...
    if (!(name in CONTROL_SCALES)) throw new Error(`${label} has an unknown control: ${name}.`);
    const scale = CONTROL_SCALES[name as ControlName] as readonly string[];
    if (!scale.includes(String(raw))) {
      throw new Error(`${label}.${name} must be one of ${scale.join(", ")}.`);
    }
  }
  return controls as Partial<AssuranceControls>;
}

export function compilePolicy(raw: unknown, source: string): CompiledPolicy {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    throw new Error("The assurance policy must be a JSON object.");
  }
  const policy = { ...DEFAULT_POLICY, ...(raw as Record<string, unknown>) } as AssurancePolicy;
  policy.floors = { ...DEFAULT_POLICY.floors, ...((raw as AssurancePolicy).floors || {}) };
  ...
  for (const key of Object.keys(raw as Record<string, unknown>)) {
    if (!known.has(key)) throw new Error(`Unknown assurance policy field: ${key}.`);
  }
  if (policy.version !== 1) throw new Error("The assurance policy version must be 1.");
  ...
  const profiles = new Map<string, AssuranceControls>();
  ...
  const resolveProfile = (name: string): AssuranceControls => {
    const existing = profiles.get(name);
    if (existing) return existing;
    const definition = policy.profiles[name];
    if (!definition) throw new Error(`Unknown assurance profile: ${name}.`);
    if (!/^[a-z][a-z0-9-]{0,31}$/.test(name)) throw new Error(`Invalid profile name: ${name}.`);
    if (resolving.has(name)) throw new Error(`Assurance profile inheritance cycle at ${name}.`);
    resolving.add(name);
    if (typeof definition.extends !== "string") throw new Error(`Profile ${name} must declare which profile it extends.`);
    ...
    const controls: AssuranceControls = {
      ...parent,
      ...overrides,
      reviewLenses: [...new Set([...parent.reviewLenses, ...(overrides.reviewLenses ?? [])])].sort() as ReviewLens[],
    };
  ...
  for (const name of Object.keys(policy.profiles)) resolveProfile(name);

  const requireProfile = (name: string, label: string): void => {
    if (!profiles.has(name)) throw new Error(`${label} references an unknown profile: ${name}.`);
  };
  const requireAtLeast = (name: string, minimum: string, label: string): void => {
    requireProfile(name, label);
    const weaker = weakerControls(profiles.get(name)!, profiles.get(minimum)!);
    if (weaker.length > 0) throw new Error(`${label} must be at least ${minimum}; ${name} is weaker on ${weaker.join(", ")}.`);
  };
  ...
  if (!Array.isArray(policy.floors.paths)) throw new Error("floors.paths must be an array.");
  ...
  return {
    ...
    profiles,
    ranks,
    definition_sha256: sha256(canonicalJson(policy as unknown as Record<string, unknown>)),
    source,
  };
}
...
export function loadPolicy(root: string): CompiledPolicy {
  ...
  if (existsSync(local)) return compilePolicy(readJson(local), POLICY_REL);
...
export interface SelectionState {
  version: 1;
  project: { selection: string; updated_at: string } | null;
  tasks: Record<string, { selection: string; updated_at: string }>;
}

export function readSelection(root: string): SelectionState {
  const path = resolve(root, ASSURANCE_STATE_REL);
  if (!existsSync(path)) return { version: 1, project: null, tasks: {} };
...
export function effectiveSelection(root: string, taskId: string | null): string {
  const state = readSelection(root);
  return (taskId && state.tasks[taskId]?.selection) || state.project?.selection || "adaptive";
}

export function writeSelection(root: string, selection: string, taskId: string | null): SelectionState {
  return withStateLock(root, "assurance", () => {
    const state = readSelection(root);
    const entry = { selection, updated_at: new Date().toISOString() };
    if (taskId) state.tasks[taskId] = entry;
    else state.project = entry;
...
export interface ResolveInput {
  ...
  risk?: RiskLevel | null;
  changedPaths?: string[];
  /** Impact classes from the module catalog that make the mapping untrustworthy. */
  impactKinds?: string[];
  /** Attributes declared on affected modules. */
  moduleAttributes?: Array<{ module: string; attribute: QualityAttribute; tier: AttributeTier }>;
}

export interface ResolvedAssurance {
  selection: string;
  /** The profile the selection asked for, before floors. */
  requested: string;
  /** The strongest profile among the request and every floor. */
  effective: string;
  /** Built-in rank the effective profile reduces to. */
  rank: BuiltinProfile;
  controls: AssuranceControls;
  floors: AssuranceFloor[];
  policy_sha256: string;
  policy_source: string;
  /** Hash of the effective controls; part of every plan hash so a profile change stales receipts. */
  assurance_sha256: string;
}

export function resolveAssurance(compiled: CompiledPolicy, input: ResolveInput): ResolvedAssurance {
  const selection = input.selection ?? "adaptive";
  const requested = selection === "adaptive" ? compiled.policy.default : selection;
  if (!compiled.profiles.has(requested)) throw new Error(`Unknown assurance profile: ${requested}.`);

  const floors: AssuranceFloor[] = [];
  const apply = (profile: string, source: string, reason: string) => floors.push({ source, profile, reason });

  if (input.risk) apply(compiled.policy.floors.risk[input.risk], "risk", `task risk is ${input.risk}`);
  ...
  for (const entry of input.moduleAttributes ?? []) {
    if (entry.tier !== "critical" && entry.tier !== "high") continue;
    const isProtected = PROTECTED_ATTRIBUTES.includes(entry.attribute);
    apply(
      isProtected ? compiled.policy.floors.protectedAttributes : compiled.policy.floors.criticalHighAttributes,
      `attribute:${entry.module}/${entry.attribute}`,
      `${entry.module} declares ${entry.attribute} at ${entry.tier}`,
    );
  }
  ...
  return {
    ...
    assurance_sha256: sha256(canonicalJson({ effective, controls: controls as unknown as Record<string, unknown> })),
  };
}

/** Adapter from a computed impact to resolver input, so every consumer derives floors the same way. */
export function assuranceForImpact(
  root: string,
  impact: ImpactResult,
  taskRisk: RiskLevel | null,
  options: { selection?: string; taskId?: string | null } = {},
): ResolvedAssurance {
  const compiled = loadPolicy(root);
  const impactKinds = new Set<string>();
  ...
  if (definition.modules.some((module) => module.shared && impact.direct.includes(module.id))) impactKinds.add("shared");
  if (impact.expansion_reasons.some((reason) => /Git is unavailable/.test(reason))) impactKinds.add("no-git");
  const moduleAttributes: ResolveInput["moduleAttributes"] = [];
  for (const module of definition.modules as ModuleDefinition[]) {
    if (!affectedIds.has(module.id)) continue;
 ...
 */
export function convenedLenses(
  controls: AssuranceControls,
  modules: Array<Pick<ModuleDefinition, "id" | "attributes">>,
): { convened: ReviewLens[]; excluded: Array<{ lens: ReviewLens; reason: string }> } {
  const declared = new Map<string, AttributeTier>();
  const order: AttributeTier[] = ["none", "minimal", "low", "medium", "high", "critical"];
  for (const module of modules) {
    for (const [attribute, requirement] of Object.entries(module.attributes || {})) {
      const tier = normalizeRequirement(requirement).tier;
      const previous = declared.get(attribute);
      if (!previous || order.indexOf(tier) > order.indexOf(previous)) declared.set(attribute, tier);
    }
  }
  const convened: ReviewLens[] = [];
  const excluded: Array<{ lens: ReviewLens; reason: string }> = [];
  for (const lens of controls.reviewLenses) {
    ...
    if (order.indexOf(tier) >= order.indexOf("low")) convened.push(lens);
    else excluded.push({ lens, reason: `no affected module declares ${attribute} above minimal` });
  }
...
export function closeLoan(root: string): boolean {
  const path = resolve(root, LOAN_REL);
...
export interface DebtEntry {
  ...
  opened_at: string;
  paid_at: string | null;
  /** Receipt content hash of the later PASS that repaid this debt. */
  paid_by: string | null;
}
...
export function readDebts(root: string): DebtFile {
...
export function openDebts(root: string): DebtEntry[] {
...
export function recordDebts(root: string, entries: Array<Omit<DebtEntry, "id" | "opened_at" | "paid_at" | "paid_by">>): DebtEntry[] {
...
export function settleDebts(
  root: string,
  receipts: Array<{ check_id: string; status: string; created_at: string; content_sha256?: string }>,
): DebtEntry[] {
  ...
  return withStateLock(root, "evidence-debt", () => {
    ...
    for (const entry of file.entries) {
      ...
      const payment = passing.find(
        (receipt) => receipt.check_id === entry.check && Date.parse(receipt.created_at) > Date.parse(entry.opened_at),
      );
      if (!payment) continue;
    ...
    if (settled.length > 0) writeJson(resolve(root, DEBT_REL), file);
    return settled;
  });
}

/** A check evidencing a protected attribute or classed as security is never deferrable. */
export function isProtectedCheck(check: { class?: string; attributes?: QualityAttribute[] }): boolean {
  if (check.class && ["security", "safety", "privacy"].includes(check.class)) return true;
...
export function profileCommand(positional: string[], options: CliOptions): void {
  ...
  if (subcommand === "list") {
    printJson({
      ...
      profiles: [...compiled.profiles.entries()].map(([name, controls]) => ({
        name,
        builtin: (PROFILE_ORDER as readonly string[]).includes(name),
  ...
  if (subcommand === "set") {
    ...
    if (!selection) throw new Error("profile set requires a profile name or `adaptive`.");
  ...
  if (subcommand === "show" || subcommand === "explain") {
    ...
    const risk = options.risk === undefined ? task?.risk ?? null : (String(options.risk) as RiskLevel);
...
export function fastCommand(positional: string[], options: CliOptions): void {
  const root = targetFrom(options);
  ...
  if (subcommand === "on") {
    ...
    printJson({
      ...
      loan,
      note:
        "The loan expires by itself. Every deferred check is recorded as SKIPPED (fast-loan) and as open debt; a loaned gate cannot close a task or a release.",
    });
    return;
  }
  if (subcommand === "off") {
...
export function debtCommand(positional: string[], options: CliOptions): void {
  const root = targetFrom(options);
  const subcommand = positional[0] || "list";
  if (subcommand !== "list") throw new Error("debt supports the list subcommand.");
  const file = readDebts(root);
  ...
  if (!includePaid && entries.length > 0) process.exitCode = EXIT.VIOLATION;
}
