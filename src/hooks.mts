...
export async function hook(event: string, options: CliOptions): Promise<void> {
  ...
  if (event === "beforeShellExecution") {
    ...
    if (existsSync(marker)) {
      try {
        ...
        output = {
          additional_context:
            "A context compaction just happened. Compaction removes governance constraints rather than diluting them, so the following was re-derived from files, not from the summary. Calibrate against it, not against the compacted impression." +
    ...
    if (payload.status === "completed" && changedThisSession) {
      ...
      if (assessment.assurance.controls.memorySync === "block") {
        try {
          ...
          if (behind) parts.push(`governed code changed but ${sync.ledger} did not (${behind.sample?.slice(0, 3).join(", ")})`);
        } catch {
      ...
      if (parts.length === 0) {
        ...
        const signature = sha256(
          canonicalJson({ base: current.base_commit, diff: current.diff_sha256, parts: [...parts].sort() }),
        );
        ...
        if (release) {
          ...
          appendLedger(
            ...
            payload,
            "followup",
            `stop-strike-release after ${STOP_STRIKE_LIMIT} blocks of the same unresolved state; control handed back without marking work complete: ${parts.join("; ")}`,
          );
          output = {
            additional_context:
              `The completion gate has blocked this same unresolved state ${STOP_STRIKE_LIMIT} times and is handing control back so the turn is not deadlocked. ` +
          ...
          output = {
            followup_message:
              `Under the ${assessment.assurance.effective} assurance profile the current diff has ${parts.join("; and ")} (block ${strikes} of ${STOP_STRIKE_LIMIT} for this unresolved state). ` +
...
function recordStopStrike(root: string, signature: string): number {
  return withStateLock(root, "stop-strikes", () => {
    const path = resolve(root, STOP_STRIKES_REL);
    let previous: { signature?: string; count?: number } = {};
    try {
      if (existsSync(path)) previous = readJson(path);
    } catch {
...
function clearStopStrike(root: string): void {
  try {
    const path = resolve(root, STOP_STRIKES_REL);
...
function planBase(current: { base_commit: string }): { base?: string } {
...
function assuranceBanner(root: string, taskId: string | null): string {
  ...
  if (debts > 0) parts.push(`${debts} evidence debt(s) are unpaid; a loaned gate cannot close a task or a release.`);
  return parts.join(" ");
}
