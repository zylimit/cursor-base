...
test("a fast loan's window is clamped on read, so a hand-edited file cannot extend or slide it", (t) => {
  const root = fixture(t);
  edit(root, "src/app.js", "export const one = 2;\n");
  const opened = jsonResult(runHarness(["fast", "on", "--minutes", "30", "--reason", "clamp probe", "--target", root]));
  assert.equal(opened.loan.minutes, 30);
  const loanPath = resolve(root, ".cursor/harness-state/fast-loan.json");
  const maxMs = jsonResult(runHarness(["fast", "status", "--target", root])).max_loan_minutes * 60_000;
  assert.ok(Number.isFinite(maxMs) && maxMs > 0, "the policy maximum must be readable");
  const tenYears = 10 * 365 * 24 * 3600 * 1000;
  const original = JSON.parse(readFileSync(loanPath, "utf8"));

  // An expires_at pushed ten years out is cut back to opened_at + the policy maximum.
  writeJson(loanPath, { ...original, expires_at: new Date(Date.now() + tenYears).toISOString() });
  const extended = jsonResult(runHarness(["fast", "status", "--target", root]));
  assert.equal(extended.active, true);
  const extendedWindow = Date.parse(extended.loan.expires_at) - Date.parse(original.opened_at);
  assert.ok(extendedWindow <= maxMs, `the window must not exceed the policy maximum: window=${extendedWindow}ms max=${maxMs}ms expires=${extended.loan.expires_at} opened=${original.opened_at}`);

  // An opened_at written in the future cannot slide the window forward: the anchor is now.
  writeJson(loanPath, {
    ...original,
    opened_at: new Date(Date.now() + tenYears).toISOString(),
    expires_at: new Date(Date.now() + tenYears + 30 * 60_000).toISOString(),
  });
  const slid = jsonResult(runHarness(["fast", "status", "--target", root]));
  assert.equal(slid.active, true);
  assert.ok(Date.parse(slid.loan.expires_at) - Date.now() <= maxMs + 5_000, "a future opened_at is anchored to now");

  // No readable opening time means no measurable window: the loan is not recognized.
  const { opened_at: _dropped, ...withoutOpened } = original;
  writeJson(loanPath, withoutOpened);
  const unrecognized = jsonResult(runHarness(["fast", "status", "--target", root]));
  assert.equal(unrecognized.active, false);
  assert.equal(unrecognized.expired, true);
});
